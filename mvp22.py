...
def _collect_waypoints_on_mesh(mesh):
    pts = []
    ...
    while True:
        gp.SetCommandPrompt(
            "Pick bar path points on mesh (Enter=finish, Esc=cancel). Picked: {}".format(len(pts))
        )
        res = gp.Get()
        ...
        if res == Rhino.Input.GetResult.Nothing:
            if len(pts) >= 2:
                return pts
            Rhino.RhinoApp.WriteLine("Pick at least 2 points before pressing Enter.")
            continue
...
def _measure_once(mesh_obj, mesh, graph, spacing):
    waypoints = _collect_waypoints_on_mesh(mesh)
